...
public class MovieServiceScheduleImpl implements MovieScheduleService {
	...
	// the method to add the movie-schedule
	@Override
	...
	// the method to get all movie-schedules list
	@Override
	...
	// the method to update the existing movie-schedule
	@Override
	...
	public void consumeKafka(Registration registration) {
		...
		if (movieScheduleRepo.findByTheatreName(registration.getTheatreName()) != null) {
			addTheatre = movieScheduleRepo.findByTheatreName(registration.getTheatreName());
			...
			addTheatre.setTheatreName(theatreId);
